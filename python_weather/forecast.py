...
from .constants import (
  VALID_FORMATS, is_invalid_format, TIME_REGEX,
  LOCAL_DATETIME_REGEX, UTC_DATETIME_REGEX, DATE_REGEX,
  LATLON_REGEX
)
...
class ModifiableFormat:
  ...
  def format(self, to: str):
    ...
    Raises:
        InvalidArg: Invalid format type.
    """
...
class BaseForecast(ModifiableFormat):
  ...
  def wind_direction(self) -> str:
    """
    Returns:
        str: The wind direction value. This can be `N`, `S`, `W`, `E`, etc.
    """

    return self.__inner['winddir16Point']
  
  @property
  def description(self) -> str:
    """
    Returns:
        str: The description regarding the forecast.
    """
...
class CurrentForecast(BaseForecast):
  ...
  def local_timezone(self) -> timezone:
    ...
    h_local, m_local, ampm_local = LOCAL_DATETIME_REGEX.findall(self._BaseForecast__inner['localObsDateTime'])[0]
    h_utc, m_utc, ampm_utc = UTC_DATETIME_REGEX.findall(self._BaseForecast__inner['observation_time'])[0]
...
class HourlyForecast(BaseForecast):
  ...
  def dew_point(self) -> int:
    ...
    return int(self._BaseForecast__inner[f'DewPoint{self._ModifiableFormat__format}'])
  ...
  def heat_index(self) -> int:
    ...
    return int(self._BaseForecast__inner[f'HeatIndex{self._ModifiableFormat__format}'])
  ...
  def wind_chill(self) -> int:
    ...
    return int(self._BaseForecast__inner[f'WindChill{self._ModifiableFormat__format}'])
...
class DailyForecast(ModifiableFormat):
  ...
  def hourly(self) -> Generator[HourlyForecast, None, None]:
    ...
    return (HourlyForecast(elem, self._ModifiableFormat__format) for elem in self.__inner['hourly'])
...
class Weather(ModifiableFormat):
  ...
  def current(self) -> CurrentForecast:
    ...
    return CurrentForecast(self.__inner['current_condition'][0], self._ModifiableFormat__format)
  ...
  def forecasts(self) -> Generator[DailyForecast, None, None]:
    ...
    return (DailyForecast(elem, self._ModifiableFormat__format) for elem in self.__inner['weather'])
